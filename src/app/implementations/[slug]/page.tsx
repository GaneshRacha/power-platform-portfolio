...
import { getPatternSourceUrl } from "@/data/sourceLinks";
...
export function generateStaticParams() {
  return implementations.map((item) => ({ slug: item.slug }));
}
...
export default async function ImplementationDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sourceUrl = getPatternSourceUrl(item.category, item.slug);
  ...
  return (
    <section className="container-shell py-16 sm:py-20">
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[1.25fr_.55fr] xl:items-end">
        <div>
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-sky-300/10 px-3 py-1 text-xs text-sky-200">{item.category}</span>
            <span className="rounded-full bg-emerald-300/10 px-3 py-1 text-xs text-emerald-200">{item.level}</span>
          </div>
          <h1 className="mt-5 text-4xl font-semibold tracking-tight text-white sm:text-5xl">{item.title}</h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">{item.summary}</p>
          <div className="mt-6 flex flex-wrap gap-2">
            {item.technologies.map((tech) => <span key={tech} className="rounded-md border border-white/10 px-3 py-1.5 text-xs text-slate-400">{tech}</span>)}
          </div>
          {sourceUrl && (
            <div className="mt-7 flex flex-wrap gap-3">
              <a href={sourceUrl} target="_blank" rel="noreferrer" className="primary-cta rounded-xl px-4 py-2.5 text-sm font-semibold">
                View source on GitHub ↗
              </a>
              <a href="https://github.com/GaneshRacha/power-platform-patterns" target="_blank" rel="noreferrer" className="secondary-cta rounded-xl px-4 py-2.5 text-sm font-semibold">
        ...
        <aside className="glass rounded-2xl p-5">
          <p className="text-xs uppercase tracking-[.18em] text-sky-300">Engineering discipline</p>
          <p className="mt-3 text-lg font-semibold text-white">{discipline?.label ?? item.category}</p>
          <p className="mt-2 text-sm leading-6 text-slate-400">{discipline?.description}</p>
        </aside>
      </div>

      <div className="mt-12 grid gap-5 lg:grid-cols-2">
        <article className="glass rounded-2xl p-6">
          <p className="text-xs uppercase tracking-[.2em] text-sky-300">Problem</p>
          <h2 className="mt-3 text-xl font-semibold text-white">Engineering challenge</h2>
          <p className="mt-4 leading-7 text-slate-400">{item.challenge}</p>
        </article>
        <article className="glass rounded-2xl p-6">
          <p className="text-xs uppercase tracking-[.2em] text-emerald-200">Implementation</p>
          <h2 className="mt-3 text-xl font-semibold text-white">Implementation approach</h2>
          <ol className="mt-4 space-y-3 text-sm leading-6 text-slate-400">
            {item.approach.map((point, i) => <li key={point} className="flex gap-3"><span className="text-emerald-300">{String(i + 1).padStart(2, "0")}</span><span>{point}</span></li>)}
          </ol>
        </article>
      </div>

      <div className="mt-5 glass rounded-3xl p-6 sm:p-8">
        <p className="text-xs uppercase tracking-[.18em] text-sky-300">Execution architecture</p>
        <h2 className="mt-3 text-2xl font-semibold text-white">How the pattern moves through the platform</h2>
        <div className="mt-7"><ArchitectureFlow steps={architecture} /></div>
      </div>

      {code && (
        <div className="mt-5 code-panel">
          <div className="code-panel-head"><span>Representative sanitized pattern</span><span>{item.category}</span></div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <article className="glass rounded-2xl p-6">
          <h2 className="text-xl font-semibold text-white">Key considerations</h2>
          <div className="mt-4 flex flex-wrap gap-2">
            {item.considerations.map((point) => <span key={point} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-slate-400">{point}</span>)}
          </div>
        </article>
        <article className="glass rounded-2xl p-6">
          <h2 className="text-xl font-semibold text-white">Design decisions</h2>
          <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-400">
            {item.designDecisions.map((point) => <li key={point} className="flex gap-3"><span className="text-sky-300">→</span><span>{point}</span></li>)}
          </ul>
        </article>
      </div>
      ...
      {matchingPlaybooks.length > 0 && (
        <div className="mt-12">
          <h2 className="text-2xl font-semibold text-white">Used in solution playbooks</h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {matchingPlaybooks.map((p) => (
              <Link key={p.slug} href={`/playbooks#${p.slug}`} className="glass card-hover rounded-2xl p-5">
                <p className="text-xs uppercase tracking-[.16em] text-sky-300">{p.eyebrow}</p>
                <h3 className="mt-3 text-lg font-semibold text-white">{p.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-400">{p.summary}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      {related.length > 0 && (
        <div className="mt-12">
          <h2 className="text-2xl font-semibold text-white">Related {item.category} patterns</h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {related.map((entry) => <ImplementationCard key={entry.slug} item={entry} />)}
          </div>
        </div>
      )}
    </section>
